...
export function buildRiskReport(
  ...
  if (volatility > 4.5 || detail.professionalScores.volatilityRisk > 70) {
    findings.push(
      finding({
        ...
        action: "Positionsgroesse und Stop-Risiko sehr konservativ pruefen."
      })
    );
  }
  ...
  if (detail.quote.volume < 5_000_000 && detail.asset.type !== "crypto") {
    findings.push(
      finding({
        id: "liquidity-low",
        category: "liquidity",
        title: "Moeglich schwache Liquiditaet",
        ...
        action: "Ordergroesse, Spread und Handelsplatz pruefen."
      })
    );
  }
  ...
  if (negativeNews.length) {
    findings.push(
      finding({
        ...
        action: "Quellen lesen und These gegenpruefen."
      })
    );
  }
    ...
    if (daysUntilEarnings >= 0 && daysUntilEarnings <= 14) {
      findings.push(
        finding({
          ...
          detail: "Ergebnisse koennen Volatilitaet und Gaps deutlich erhoehen.",
  ...
  if (detail.quote.changePercent > 7 && !positiveNews) {
    findings.push(
      finding({
        ...
        category: "pump-dump",
        title: "Pump-and-Dump-Verdacht pruefen",
        severity: "hoch",
        ...
        action: "Social-Media-Hype, Liquiditaet und News-Ursache pruefen."
      })
    );
  }
  ...
  if (detail.indicators.rsi > 70 || detail.indicators.rsi < 30) {
    findings.push(
      finding({
        ...
        category: "technical",
        title: detail.indicators.rsi > 70 ? "Ueberkaufter RSI" : "Ueberverkaufter RSI",
        severity: detail.indicators.rsi > 82 || detail.indicators.rsi < 18 ? "hoch" : "mittel",
        detail: "RSI-Extreme koennen Trendstaerke oder Rueckschlagrisiko anzeigen.",
  ...
  if (detail.quote.price < detail.indicators.support[0]) {
    findings.push(
      finding({
        ...
        action: "Breakdown-Szenario und Fehlsignal pruefen."
      })
    );
  }
  ...
  if (detail.macroFactors.some((factor) => factor.impact === "negative")) {
    findings.push(
      finding({
        ...
        severity: "mittel",
        detail: "Mindestens ein Makro-Faktor wird als moegliches Risiko markiert.",
        evidence: detail.macroFactors.filter((factor) => factor.impact === "negative").map((factor) => factor.label).join(", "),
        action: "Marktregime, Zinsen und Liquiditaet getrennt pruefen."
      })
    );
  }
  ...
  if (detail.analysisLayers.some((layer) => layer.label === "Sektortrend" && layer.status === "negative")) {
    findings.push(
      finding({
        ...
        action: "Relative Staerke gegen Sektor und Index vergleichen."
      })
    );
  }
  ...
  if (!dataQuality.sufficientForAnalysis) {
    findings.push(
      finding({
        ...
        evidence: [...dataQuality.issues, ...dataQuality.warnings].join(" | ") || `Qualitaet ${dataQuality.score}/100.`,
        action: "Daten aktualisieren und zusaetzliche Quellen hinzuziehen."
      })
    );
  }
  ...
  return {
    ...
    summary:
      findings.length === 0
        ? "Keine kritischen Warnungen im Modell erkannt. Das ersetzt keine eigene Pruefung."
