...
select has_column('public', 'account_deletion_jobs', 'user_fingerprint', 'jobs retain a pseudonymous deletion lookup');
...
select is(length((select user_fingerprint from public.account_deletion_jobs limit 1)), 64, 'claim records a SHA-256 fingerprint');
select is(public.record_account_deletion_step(
  (select id from public.account_deletion_jobs where user_id='11111111-1111-4111-8111-111111111111'),
  'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
  'completed', 'invalid_jump', '{}', null, null, null
), false, 'invalid state jump is rejected');
select is((select status from public.account_deletion_jobs limit 1), 'cancelling_subscriptions', 'invalid jump leaves state unchanged');
reset role;
...
select ok(public.record_account_deletion_step(
  (select id from public.account_deletion_jobs where user_id='11111111-1111-4111-8111-111111111111'),
  'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
  'deleting_identity', 'subscriptions_cancelled', '{}', null, null, null
), 'claimed worker enters identity deletion');
select ok(public.record_account_deletion_step(
  (select id from public.account_deletion_jobs where user_id='11111111-1111-4111-8111-111111111111'),
  'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',
...
select is(length((select user_fingerprint from public.account_deletion_jobs where status='completed')), 64, 'completed tombstone retains pseudonymous lookup');
select is((select status from public.account_deletion_jobs limit 1), 'completed', 'completed status persists');
select is((select count(*) from public.account_deletion_events), 3::bigint, 'completion appends audit evidence');
...
select ok((select purge_after >= completed_at + interval '179 days' from public.account_deletion_jobs limit 1), 'retention starts when deletion completes');
reset role;
