...
| §4 | Zentrale Entitlement-Definition | `DONE` | `feature-gates.ts` + `billing/entitlements.ts`, 23 Features, **6 Limits — jedes mit Durchsetzungsort** |
...
| §4 | Limits `maxWatchlists`, `maxSavedScreeners` | `DONE` | **Entfernt.** Sie begrenzten Funktionen, die es nicht gibt: `watchlists` ist eine flache Tabelle `(user_id, symbol)` ohne benannte Listen, gespeicherte Screener existieren nirgends. Ein Test verhindert das Zurückschleichen |
...
| §28 | Makro | `DONE` | **36 Reihen konfiguriert**: 13 EZB (Euroraum), 23 FRED (USA), dazu 5 Marktindikatoren. ECB liefert SDMX-Historie mit `VALID_FROM`/`VALID_TO`; FRED liefert Werte ohne Schlüssel per offiziellem CSV und mit `FRED_API_KEY` zusätzlich Vintages. Alles aus §28 außer **PMI**. Offen bleibt die Lizenzfrage |
...
| §37 | Bewertungsmodelle DCF/Multiples | `DONE` | DCF, Reverse DCF, Sensitivität, Gewinn- und FCF-Rendite, Peer-Vergleich und **historische Bewertung** über den Fünfjahresmedian. Auf der Asset-Seite verdrahtet. 34 Tests, fünf Regressionen |
